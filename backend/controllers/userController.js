...
const createUser = asyncHandler(async (req, res) => {
    ...
    if (userExists) res.status(400).send("User already exists with this email");
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    if (existingUser) {// Compare password
        const isPasswordValid = await bcrypt.compare(password , existingUser.password)// Create token

        if(isPasswordValid){// Send response
